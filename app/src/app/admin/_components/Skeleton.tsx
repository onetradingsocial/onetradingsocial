...
export function Block({ h, w = '100%', r = 12 }: { h: number; w?: string | number; r?: number }) {
  return <span className="ad-skel" style={{ height: h, width: w, borderRadius: r, display: 'block' }} />
}

/**
 * Page title + subtitle, matching .ad-head metrics.
 * `right` mirrors PageHead's right slot (badge, toggle, tab bar); `subLines`
 * matches how far the real sub paragraph wraps — .ad-head p allows 74ch.
 */
...
/** Segmented filter stand-in — mirrors .ad-tabs. */
export function TabsSkeleton({ count = 4 }: { count?: number }) {
  return (
    <div className="ad-tabs" aria-hidden>
      {Array.from({ length: count }, (_, i) => (
        <span key={i} className="ad-tab" style={{ display: 'flex', alignItems: 'center' }}>
 ...
 * `stacked` switches to .ad-row-stack: a meta line plus a wrapped paragraph,
  ...
  return (
    <div className="ad-panel" aria-hidden>
      ...
      {Array.from({ length: rows }, (_, i) => (
        stacked ? (
          <div key={i} className="ad-row ad-row-stack">
 ...
 * `title` adds the .ad-panel-head bar that <Panel title=…> emits, `scroll`
 * matches the .ad-panel-scroll wrapper, and `widths` mirrors a <colgroup>.
 */
export function TableSkeleton({ rows = 6, cols = 5, title = false, scroll = true, widths }: {
  ...
  return (
    <div className="ad-panel" aria-hidden>
      {title && <div className="ad-panel-head"><Bone w={120} variant="pill" /></div>}
      {scroll ? <div className="ad-panel-scroll">{table}</div> : table}
    </div>
  )
}
...
export function MetersSkeleton({ rows = 4, title = false }: { rows?: number; title?: boolean }) {
  return (
    <div className="ad-panel" aria-hidden>
      {title && <div className="ad-panel-head"><Bone w={140} variant="pill" /></div>}
        ...
        {Array.from({ length: rows }, (_, i) => (
          <div key={i} className="ad-meter">
            ...
            <div className="ad-meter-track">
              <span className="ad-skel ad-skel--bar" style={{ width: `${Math.max(8, 84 - i * 9)}%`, display: 'block' }} />
            </div>
          </div>
        ))}
      </div>
...
export function BarsSkeleton({ bars = 12 }: { bars?: number }) {
  return (
    <div className="ad-panel" aria-hidden>
      <div className="ad-panel-head"><Bone w={130} variant="pill" /></div>
      <div className="ad-panel-body">
        <div className="ad-chart" style={{ margin: 0 }}>
          <div className="ad-chart-y" style={{ height: 92, marginTop: 16 }}>
          ...
          <div className="ad-chart-plot">
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${bars}, minmax(0, 1fr))`, columnGap: '0.3%', alignItems: 'end', height: 108 }}>
              {Array.from({ length: bars }, (_, i) => (
                <span key={i} className="ad-skel" style={{ height: `${28 + ((i * 37) % 62)}%`, borderRadius: 2, display: 'block' }} />
              ))}
            </div>
          </div>
          <div className="ad-chart-x" style={{ gridTemplateColumns: `repeat(${bars}, minmax(0, 1fr))`, columnGap: '0.3%' }}>
            {Array.from({ length: bars }, (_, i) => (
